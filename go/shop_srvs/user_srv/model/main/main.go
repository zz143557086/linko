...
func main() {
	dsn := "root:123456@tcp(192.168.0.104:3306)/shop_user_srv?charset=utf8mb4&parseTime=True&loc=Local"
	...
	for i := 0; i < 10; i++ {
		password := genMd5(fmt.Sprintf("12345%d", i))
		user := model.User{
			Name:     fmt.Sprintf("lin%d", i),
			Mobile:   fmt.Sprintf("1552005043%d", i),
			Password: password,
		}
		db.Save(&user)
	}
