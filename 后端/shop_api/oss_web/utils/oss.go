...
func Get_policy_token() string {
	...
	condition = append(condition, global.OssUploadDir)
	config.Conditions = append(config.Conditions, condition)

	//calucate signature
	result, err := json.Marshal(config)
	debyte := base64.StdEncoding.EncodeToString(result)
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(global.OssApiSecrect))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))

	var callbackParam CallbackParam
	callbackParam.CallbackUrl = global.CallBackUrl
	...
	policyToken.AccessKeyId = global.OssApiKeyId
	policyToken.Host = global.OssHost
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = global.OssUploadDir
